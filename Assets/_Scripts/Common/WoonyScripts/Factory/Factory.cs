...
public class Factory
{
    ...
    {
        defaultPoolSize = Math.Max(defaultPoolSize, 1);
    ...
    public RecycleObject Get()
    {
        if (_pool.Count <= 0)
        {
            ...
            else _usingPool.GetLastItem().Restore();
        }

        _tempObject = _pool.Dequeue();

        OnObjectGet(_tempObject);

        _tempObject.OnDequeueByFactory();
    ...
    protected virtual void OnObjectGet(RecycleObject tempObject) { }
    ...
    protected virtual void OnObjectRestore(RecycleObject tempObject) { }
}